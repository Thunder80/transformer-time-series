...
def create(indicator_name, stock_name, stock_symbol):
    ...
    data[indicator_name.capitalize()] = talib.CDLDOJI(data['Open'], data['High'], data['Low'], data['Close'])

    doji_patterns = data[data[indicator_name.capitalize()] == 100]
    print(doji_patterns)
...
for stock in stocks:
    create("doji", stock["name"], stock["symbol"])
