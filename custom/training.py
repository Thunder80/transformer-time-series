from custom.predict import predict, predict_multi
...
def train_multi_model(model, train_loader_daily, train_loader_weekly, time_series_data, time_series_data_weekly, criterion, optimizer, num_epochs, input_sequence_length, output_sequence_length, feature_size, device, root_folder, daily_features, weekly_features, probability_decrease = 0.1, decrease_rate = 0.1):
    min_loss = 500
    decrease_after = num_epochs * decrease_rate
    losses = []
    scaler = load(f"{root_folder}/joblib/scaler.joblib")
    ...
    for epoch in tqdm(range(num_epochs),  desc="Training: "):
        ...
        it = iter(train_loader_weekly)
        for batch_data, batch_targets in train_loader_daily:
            optimizer.zero_grad()

            random_number = random.random()

            batch_data_weekly, batch_targets_weekly = next(it)

            tgt_daily = torch.zeros(batch_targets[:, :-1, :].shape, device=device)
            if random_number < probability_thresold:
                tgt_daily = batch_targets[:, -output_sequence_length+1:, :]
                sampled_count += 1
            else:
                non_sampled_count += 1

            tgt_weekly = torch.zeros(batch_targets_weekly[:, :-1, :].shape, device=device)
            if random_number < probability_thresold:
                tgt_daily = batch_targets_weekly[:, -output_sequence_length+1:, :]
                
            predictions = model(batch_data, tgt_daily, batch_data_weekly, tgt_weekly)
        ...
        if epoch % 10 == 0:
            prediction_ind, all_predictions, test_loss = predict_multi(model=model, time_series_data=time_series_data, criterion=criterion, input_sequence_length=input_sequence_length, output_sequence_length=output_sequence_length, feature_size=feature_size, device=device, root_folder=root_folder, daily_features=daily_features, weekly_features=weekly_features, time_series_data_weekly=time_series_data_weekly)
    ...
    print("Training finished!")
    prediction_ind, all_predictions, total_loss = predict_multi(model=model, time_series_data=time_series_data, criterion=criterion, input_sequence_length=input_sequence_length, output_sequence_length=output_sequence_length, feature_size=feature_size, device=device, root_folder=root_folder, daily_features=daily_features, weekly_features=weekly_features, time_series_data_weekly=time_series_data_weekly)
